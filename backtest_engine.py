...
class BacktestEngine:
    def __init__(self, session_id: str, starting_balance_usd: float | None = None):
        self.session_id = session_id
        self.balance_usd = starting_balance_usd or CONFIG.risk.starting_balance_usd
        self.gecko = GeckoTerminalClient()
        self.logger = TradeLogger(log_path="logs/backtest_trades.csv")

    def run_pool(self, pool: dict, timeframe: str = "hour", aggregate: int = 1, candle_limit: int = 1000) -> int:
    ...
    def run(self, pools: list[dict], timeframe: str = "hour", aggregate: int = 1) -> dict:
        for pool in pools:
            try:
                self.run_pool(pool, timeframe=timeframe, aggregate=aggregate)
